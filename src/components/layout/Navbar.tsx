  ...
  return (
    <nav className="h-16 px-4 sm:px-8 border-b border-slate-200 bg-white flex items-center justify-between flex-shrink-0 sticky top-0 z-50">
      ...
      <div className="hidden md:flex items-center gap-4">
        <button className="px-4 py-2 text-sm font-semibold text-slate-700 bg-slate-100 rounded-full hover:bg-slate-200 transition-colors">Sign In</button>
        <button className="px-4 py-2 text-sm font-semibold text-white bg-emerald-600 rounded-full shadow-sm shadow-emerald-200 hover:bg-emerald-700 transition-colors">Post a Listing</button>
      </div>
      ...
      <AnimatePresence>
        {isOpen && (
          ...
          >
            ...
            <button className="w-full mt-4 bg-emerald-600 text-white px-5 py-4 rounded-full font-bold">Post a Listing</button>
          </motion.div>
        )}
      </AnimatePresence>
    </nav>
